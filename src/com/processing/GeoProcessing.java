...
public class GeoProcessing {
	...
	// Methodes
	/**
	 * Calcul de la distance entre deux points avec les coordonnées
	 * géographiques
	 * 
	 * @param lat1d
	 *            Double
	...
	/**
	 * Permet de recuperer les coordonnées des coins Nord Ouest et Sud Est Carré
	 * qui inscrit le cercle du rayon de recherche
	 * 
	 * @param latitudeDeg
	 *            Double
	 * @param longitudeDeg
	...
	public static Borders getWGS84FrameLimits(double latitudeDeg, double longitudeDeg, int rayon) {
		...
		double newLatRadNO = Math.asin(Math.sin(latitudeRad) * Math.cos(distanceAngulaireRad)
				+ Math.cos(latitudeRad) * Math.sin(distanceAngulaireRad) * Math.cos(azmRadNO));
		// Calcul de la Longitude du Point du cadre Nord Ouest
		double newLongRadNO = longitudeRad
				+ Math.atan2(Math.sin(azmRadNO) * Math.sin(distanceAngulaireRad) * Math.cos(latitudeRad),
						Math.cos(distanceAngulaireRad) - Math.sin(latitudeRad) * Math.sin(newLatRadNO));
		// Conversion rad en deg
		...
		double newLatRadSE = Math.asin(Math.sin(latitudeRad) * Math.cos(distanceAngulaireRad)
				+ Math.cos(latitudeRad) * Math.sin(distanceAngulaireRad) * Math.cos(azmRadSE));
		// Calcul de la Longitude du Point du cadre Sud Est
		double newLongRadSE = longitudeRad
				+ Math.atan2(Math.sin(azmRadSE) * Math.sin(distanceAngulaireRad) * Math.cos(latitudeRad),
						Math.cos(distanceAngulaireRad) - Math.sin(latitudeRad) * Math.sin(newLatRadSE));
		// Conversion rad en deg
